...
qboolean check_readable_file(const char *filepath)
{
    ...
    fh = fopen(filepath, "r");
    if (fh == NULL) {
        gi.dprintf("Error: File \"%s\" does not exist or cannot be opened.\n", filepath);
        return false;
    }
    ...
    size = ftell(fh);
    if (size == 0) {
        gi.dprintf("Error: File \"%s\" is empty.\n", filepath);
        fclose(fh);
        return false;
    }
    fseek(fh, 0, SEEK_SET);
	...
	if (fh == NULL) {
		gi.dprintf("Error: File \"%s\" is not readable.\n", filepath);
		return false;
	} else {
		fclose(fh);
		return true;
    ...
    // Additional file sanity checks can be added here
